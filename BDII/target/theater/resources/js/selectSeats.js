...
function storeSelectedSeatsInHiddenInputs(divId) {
  var div = document.getElementById(divId);
  for (var i = 0; i < sectorsArray.length; i++) {
    var input = document.createElement("INPUT");
    input.type = "hidden";
    input.name = "seats";
    var sectorSelectedSeatsStringInfo = sectorsArray[i].name + "," + selectedSeats[sectorsArray[i].name].toString();
    input.value = sectorSelectedSeatsStringInfo;
    console.log(sectorSelectedSeatsStringInfo);
    div.appendChild(input);
  }
}

function createHiddenInputWithRealizationId(divId, realizationId) {
  var div = document.getElementById(divId);
  var input = document.createElement("INPUT");
  input.type = "hidden";
  input.name = "realizationId";
  input.value = realizationId;
  div.appendChild(input);
}

function submitToTickets() {
  storeSelectedSeatsInHiddenInputs("ticketsFormData");
  createHiddenInputWithRealizationId("ticketsFormData", jsRealizationId);
  document.getElementById("toTicketsForm").submit();
}

function submitToReservation() {
  storeSelectedSeatsInHiddenInputs("reservationFormData");
  createHiddenInputWithRealizationId("reservationFormData", jsRealizationId);
  document.getElementById("toReservationForm").submit();
}